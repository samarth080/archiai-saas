...
Full, detailed task-by-task record of every completed sprint (0 through 16). This is the archived detail — `CLAUDE.md`'s "Sprint Progress" section keeps only a one-line summary per sprint plus a link here, so the active file stays readable. The currently active sprint (17+) lives in full in `CLAUDE.md`, not here.
...
> Full spec: [`docs/superpowers/specs/2026-06-11-sprint16-unified-space-filling-layouts.md`](superpowers/specs/2026-06-11-sprint16-unified-space-filling-layouts.md)
