...
- [ ] Phase 1 — subdivision engine v0 + hard validator (Hypothesis property gate) + debug SVG renderer
- [ ] Phase 2 — extraction (`chat_structured`, semaphore=1, normalizer, retry loop; ≥8/10 golden gate) · Phase 3 — clarification · Phase 4 — API/DB wiring · Phase 5 — 2D SVG editor · Phase 6 — scorer + data-driven `vastu_rules.json` · Phase 7 — two-way sync + 90° rotation · Phase 8 — server-side PNG/PDF + share pinning + DXF/IFC (deps `cairosvg`/`ezdxf`/`ifcopenshell` to be flagged when added) · Phase 9 — hardening
