...
- *Slice 4 — daylight / external-wall awareness (measurement):* `score_graph_satisfaction` now also measures, per generated layout, how many spaces that need an exterior wall (`Node.requires_external_wall`, already set for bedrooms/living/office/etc.) actually reach the building perimeter — surfaced in `metadata.graphSatisfaction` (`daylightTotal`/`daylightSatisfied`/`daylightMissing`). Pure measurement + explainable warning. It immediately earned its keep: it caught that the **BSP back-partition landlocks rooms** (a 3-BR house scored daylight 5/8 with rooms fully interior), while office/school scored full marks.
- *Slice 5 — daylight-aware selection (the fix the metric exposed):* added a landlocked-daylight **penalty in the quality scorer** (`_DAYLIGHT_ROOM_TYPES` that don't reach the footprint perimeter, capped −15) — so a candidate that strands a bedroom/office scores lower and loses the competition to a perimeter-friendly one. The 3-BR house now generates via the tiler at daylight **8/8** (BSP dropped to q61 vs tiler q66) instead of BSP at 5/8. Service/circulation rooms are deliberately excluded — they are what *should* take the interior on a deep plan. This surfaced a priority conflict (the graph candidate satisfies an explicit MUST but landlocks one other room, dipping its quality), resolved by letting a graph candidate win within a small quality tolerance (`_GRAPH_MUST_QUALITY_TOLERANCE = 6`): an explicit user adjacency beats a modest daylight cost, which is still warned about.

No existing candidate is generated differently and none removed. Full backend suite 514 passed, zero regressions across the 400+ existing layout/quality/benchmark tests (the scorer change only lowered scores for genuinely landlocked layouts).
...
- *T4.5 alignment + band order:* `planning/alignment.py` snaps cut coordinates and merges near-collinear cut lines (cleaner walls, fewer partition segments; tiling preserved); the band order now mirrors the tiler (entry/foyer/lobby lead, public follows, circulation threads the middle, private/service to the back).
- *T4.7 WIRED:* the slicing tree is live as a new `gtree` placement style, **additive** alongside the row-packer `graph` — both compete, both gated on hard MUST. Turning it from losing to winning took a pattern-rule-zone fix (band assignment + emitted room zone now match the scorer), fair measurement through the full door/marker pipeline, and a footprint-from-aligned-rooms fix (grid-snapping could nudge an edge past the pre-alignment depth).

**Phase 4 CLOSE-OUT (2026-07):**

- *Graph scoring & satisfaction (`planning/graph_scoring.py`):* deterministic AABB measurement of a *generated* layout vs its ProgramGraph — MUST/SHOULD shared-wall satisfaction, weighted score, unmet-MUST list, and daylight (spaces needing an exterior wall that reach the perimeter). Surfaced in `metadata.graphSatisfaction`.
- *Row-packer `graph` vs `gtree`:* both cluster MUST-linked rooms. `graph` (`_graph_pack_rooms`) flows the adjacency-ordered room set into width-filling rows (1-D). `gtree` is the full guillotine slicing tree (2-D): clusters → zone bands → recursive min-cut per band → inner recursion → alignment. `gtree` handles cross-zone MUST + external-wall routing + proportion by construction; `graph` is simpler and occasionally still best on small single-MUST programs.
- *gtree pipeline (Stages 1-8):* `prepare` (unify edges, MUST-over-AVOID, pattern-rule zones) → `contract` (MUST components → cluster super-nodes) → `place_floor` (bands front-to-back, guillotine tree per band via `slicing_tree.build_tree`, recurse into clusters) → `alignment` (snap + merge collinear cuts) → footprint from aligned extents → legacy room-dict serialization. Colours/ids injected by `layout_service` to stay import-acyclic.
- *Selection safety:* winner = highest quality among tile/BSP/graph/gtree; a graph candidate replaces that baseline **only** when genuinely better — strictly more MUST at ≤ `_GRAPH_MUST_QUALITY_TOLERANCE` (6) quality cost, **or** equal-or-more MUST at strictly higher quality. It never wins by dropping a MUST or on a tie, so an explicit user adjacency beats a modest quality dip while a worse layout can't ride in.
- *Determinism:* identical input → identical winner/score (all enumeration over sorted ids; ties to the smallest id set; stable fallback colour hash — never `hash()`). Room ids are uuids as in every engine; layout *structure* is deterministic (pinned by a benchmark double-run test).
- *Fallback:* a graph candidate that raises is omitted from the pool — tile/BSP always exist, so generation never 500s; a base-engine failure still re-raises.
- *Known limitations:* min-dim compliance is best-effort (a deep program can yield thin rooms — benchmark shows 1-5 sub-1.5 m rooms on some gtree layouts, no hard failures); AVOID is enforced within a band but not across band boundaries; `preferred_relative_position` isn't honoured (the parser drops direction); candidate seeds (T4.8) and non-guillotine/L-shaped footprints are out of scope.
- *Benchmark findings (`tests/test_phase4_benchmark.py`, final post-processed candidates):* across office/clinic/house/restaurant/warehouse-office, **every** engine passes the hard invariants (0 overlaps, 0 outside-footprint, 0 unreachable-after-doors); `gtree` wins all five on quality (93/97/84/97/87) while realising every MUST and full-or-near-full daylight (e.g. house daylight 9/9 vs BSP 8/9). The row-packer still wins narrow single-MUST cases (e.g. "reception next to consultation", q82 vs gtree 76).
