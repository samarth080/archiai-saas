...
- *T4.7 WIRED and WINNING:* the slicing tree is live as a new `gtree` placement style, **additive** alongside the row-packer `graph` — both compete, both gated on hard MUST, **zero regression** (the row-packer still wins where it wins; the safe fallback omits a graph candidate on any error, never a 500). Three fixes turned it from losing to winning: (a) `prepare` uses the pattern-rule zone so band assignment + emitted room zone match the scorer (was a systematic zone-mismatch penalty); (b) fair measurement through the full door/marker pipeline instead of raw `place_floor` (whose door-less output was docked −20 per room on the reachability check); (c) selection picks the *best* graph candidate by (MUST realised, quality), not the first MUST-improver. Result: *"office where the workspace is next to the reception"* → **gtree wins at q90** (beating tile 78 / bsp 73 / row-packer 72) while realising the MUST; *"clinic where reception is next to consultation"* → row-packer still wins (q82). ~40 engine tests; full suite 548 passed.
- *Remaining (optional polish):* T4.8 candidate seeds (rotated/alt-order/deep-service diversity), T4.11 committed benchmark fixtures, AVOID-across-band-boundary enforcement, and the doc extension. The engine is now a shipping, competing part of generation.
