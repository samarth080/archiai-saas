...
- [ ] **Not done:** 5.2 (simulated-annealing refinement over the winning candidate's decision tuple) and 5.3 (`DoesNotFitError` becomes a negotiation with automatic degradation suggestions) — both explicitly out of scope for this slice, not attempted. `generate_candidates` is also not wired into any API route yet — it exists as a tested, usable capability, not a live user-facing feature.
