...
### Door connectivity fix — every adjacent room pair gets a door, not just the spanning tree (`phase3/layout-archetypes`, stacked on the PlanRoom.type migration)

> User-reported: generated layouts feel disconnected "not always" — investigated with a live measurement before writing any code, per the usual discipline here.

- [x] **Measured the actual defect, not guessed at it:** wrote a diagnostic comparing every room pair that geometrically shares a wall against which pairs actually have a door. On `4bhk.json`: **28 room pairs share a wall, only 15 had a door** — `_place_doors`'s BFS spanning tree only ever adds the minimum doors needed for bare graph reachability, so a room stays "reachable" (passes the `unreachable` hard check) while a wall it visibly touches its neighbour through stays solid. Concretely: Dining directly touching Entry had no door, routed instead through the Living Room — exactly the kind of thing that reads as "not connected" in the 3D/2D view even though nothing was technically broken by the existing hard-constraint definition.
- [x] **Fix, not a rewrite:** `_place_doors` gains a third pass after the spanning tree: door every remaining adjacent pair with a wide-enough shared wall, *except* when both rooms are private/service-zoned (bedroom-bedroom, bedroom-bathroom — skipping these is privacy, not a bug) or the pair is explicitly in `spec.avoid_adjacency`. This is purely additive (more doors, never fewer), so it can't regress anything the spanning tree already guaranteed.
- [x] **Not a new philosophy — ported a proven one:** the legacy `layout_service.py` engine already solved exactly this for its own placement path (`test_interior_doors.py`'s docstring: "every shared wall between two adjacent rooms long enough to fit a doorway now gets a door marker") and, per the Phase 2 circulation write-up above, already has a `_wants_direct_door` heuristic distinguishing open-plan-public-connects-directly from private-cells-route-through-a-corridor-instead. This change brings the newer MVP engine's door policy in line with that established, already-tested precedent rather than inventing new rules.
- [x] **Threaded `zone_of` into `_place_doors`,** which didn't have it before: `generate_plan` passes `program.zone_of` directly; `rebuild_derived_geometry` (which rebuilds from edited `PlanRoom`s with no graph available) gets a new lenient `catalog.zone_for()` lookup, mirroring `min_dimensions`'s established never-raises pattern. Renamed `archetypes.py`'s private `_macro_zone`/`_MACRO_ZONE` to public `macro_zone`/`MACRO_ZONE` (matching this repo's own established "rename instead of reaching into another module's underscores" convention — `program_completion.py`'s `add_requirements_node` set the precedent) so `engine.py` reuses the exact same public/private folding `zoned_bands` already relies on, instead of re-deriving it.
- [x] **Re-measured after the fix, not just re-tested:** `4bhk.json` goes from 15/28 adjacent pairs doored to **18/28** — every one of the 10 remaining gaps is a private-private pair (bedroom-bedroom, bedroom-bathroom, bathroom-bathroom, study-utility), all privacy-sensible to skip, none of them the "two public rooms touching with a solid wall between them" defect this was meant to fix.
- [x] **Tests:** 7 new — every fixture's public/circulation-adjacent room pairs get a door (parametrized across all 5 fixtures); two adjacent private rooms stay undoored without breaking overall validity; an explicitly avoided pair — confirmed genuinely, non-vacuously adjacent first, not assumed — gets no direct door.
- [x] **Verification:** `pytest app/tests -q` — **829 passed** (822 baseline + 7 new), 3 expected live-LM skips, 0 failed, including the 200-example Hypothesis property gate re-run in isolation. No schema/contract change, so the frontend is unaffected by construction (more entries in an existing `Door[]` array, nothing new in shape).
- [ ] **Known remaining limit, not addressed here:** this is still door-policy-only, not real circulation geometry — there is still no actual corridor room/void (Phase 4 of the engine generalization workflow), so a very deep or unusually shaped layout could still produce a long walking path even with every adjacent pair doored. This fix closes the "visibly touching rooms with a solid wall between them" defect specifically; it is not the full circulation rewrite the roadmap describes.

### Legacy engine — the same connectivity defect, a different root cause (`phase3/layout-archetypes`)

> The user's concrete example ("3bhk house with 2 washrooms — one connected to a room, the other connected to nothing") turned out to be the **legacy** `layout_service.py` engine, not the MVP engine fixed above — most real user-facing prompt generations still route through it (`designs/router.py`'s `/api/design/generate`, the advanced `parse_prompt` path). Diagnosed by direct reproduction, not guesswork: tried the literal reported prompt shape first, then synthesized the exact failing geometry once the pattern was clear.

- [x] **Root cause confirmed by direct reproduction**, not inferred from reading code alone: `_generate_partition_walls`'s "access guarantee" (step 2) and "connectivity completion" spanning tree (step 3) both draw their candidate pool *exclusively* from `doorable` — boundaries `>= _MIN_DOORWAY_SPAN` (1.2 m). Walls themselves get drawn down to `_MIN_WALL_SPAN` (0.3 m), so a room whose every real shared wall falls between 0.3 m and 1.2 m (a narrow bathroom tucked into a corner, very plausible geometry) is **invisible to the entire door-placement pass**, not merely deprioritized — confirmed with a synthetic case (a bathroom sharing 1.0 m walls with two different neighbours) that reproduced exactly zero doors before the fix.
- [x] **Fix:** both passes gain a fallback tier, `_NARROW_DOORWAY_SPAN = 0.7 m` — used only when the comfortable `>=1.2 m` tier has nothing for that room/component — mirroring the MVP engine's own already-established narrow-door fallback (`_NARROW_DOOR_WIDTH`/`_NARROW_DOOR_EDGE`) instead of inventing a new number. `add_door` already clamps door width to the available span, so this produces a proportionally narrower door, not an oversized one.
- [x] **Explicitly not "door every shared wall"** — the user's own stated preference, matched by what was already true before this fix and remains true after it: the existing category-based policy (corridor connects to everything it touches, open-plan public rooms connect directly to each other, a private/service cell gets exactly one access door by neighbour priority) is completely unchanged. Only the candidate *pool* gained a fallback tier, so that existing policy can no longer silently skip a room for want of a few centimetres of wall.
- [x] **Tests:** the exact narrow-bathroom repro pinned directly against `_generate_partition_walls`; a real `generate_layout("3bhk house with 2 washrooms")` call asserted to produce zero rooms with no door at all. A broader stress sweep across 10 varied prompts (5bhk, clinics, duplexes, tiny/huge plots — checking every returned candidate, not just the winner) found no further zero-door rooms.
- [x] **Verification:** `pytest app/tests -q` — **831 passed** (829 baseline + 2 new), 3 expected live-LM skips, 0 failed. Frontend unaffected (no schema/contract change): 271 passed.
- [ ] **What the user also asked for and is NOT done here:** real corridor-based circulation ("continue building with corridor with proper connection to other rooms") — the legacy engine already has meaningful corridor support (`_TILED_CORRIDOR_TYPES`, the "corridor connects to everything it touches" policy already exercised by `test_private_service_cells_route_through_the_corridor_not_each_other`), and the MVP engine's equivalent is the still-unstarted Phase 4 of the engine generalization workflow. This slice fixed the specific, confirmed, reported defect (a room silently getting zero doors); it is not a circulation-design upgrade.

### Phase 4 — Circulation as first-class geometry ✅ Complete (`phase3/layout-archetypes`)

> Done autonomously per explicit instruction ("run without asking questions... make sure the codebase doesn't blow up") after the user asked whether the engine generalization workflow doc was still being followed. All 5 doc sub-items (4.1–4.5) implemented, each verified against the real fixtures and the Hypothesis property gate before moving to the next — this is what actually gives the MVP engine real corridor rooms, not just better door-connectivity heuristics (the earlier "door connectivity" entries above).

- [x] **4.1 Need rule:** `program_completion.ensure_corridor` injects a single corridor node when the program has enough genuinely-needing-it rooms and none already exists. Built additively first (not wired in), verified against real fixture impact directly, **then** wired into `engine.py::_build_program`. Threshold started at the doc's literal "≥3", was lowered to "≥2" after going live — matches the pre-existing `planning/validation.py` "no circulation" warning's own threshold (which the doc cites as this rule's precedent) and closes a real counterexample the property gate found at 3. Gated on an actual corridor/hallway/passage node, not "any circulation-type node" — the same false-positive class already caught for the Phase 3.2 archetype selector (entry is always circulation-typed and always present, so a naive check would never fire).
- [x] **4.2 Width rule:** `catalog.CIRCULATION_WIDTHS` (residential 1.0 m / commercial 1.5 m / accessible 1.8 m — only "residential" actually consumed yet, the other two are named and ready for when a building-type/accessibility context threads through). `corridor` added to `BASE_SIZES`; corridor/hallway get an explicit narrow-and-long minimum instead of the generic near-square derivation formula.
- [x] **4.3 Carving:** `zoned_bands` and `double_loaded_corridor` both carve the corridor as a real band/spine (populating `corridor_rects`, empty since 3.1a) instead of folding it into "public" like every other circulation type. Reworked twice after going live: first to comb-arrange only genuinely private rooms (`catalog.privacy_level_for >= 2`) plus their MUST-attached partners as one cluster — not every non-public room — after combing bathrooms/utility/parking alongside actual bedrooms made several real fixtures stop fitting; then to balance clusters between wings by area (not alternating index) and to comb-split a multi-room cluster's own rect too, since the general recursive `subdivide()` doesn't always find a valid cut for an oddly-shaped allocated rect even with enough total area.
- [x] **4.4 Door policy:** `_circulation_key` prefers a real corridor/hallway spine over the old living-room-or-entry heuristic; the BFS spanning tree's pair ordering now ranks circulation-to-room > public-to-public > everything else; a `separated`/AVOID edge now vetoes a door even when it's the only route left, verified with a synthetic case that correctly raises `DoesNotFitError` instead of silently punching a door through an explicit avoidance.
- [x] **4.5 Privacy-chain hard check:** `hard_constraints.py` gains `through_room_access` — no `privacy_level>=2` room may be reachable only through another such room, except a MUST-attached service (an ensuite through its own bedroom), implemented as a re-walk of the door graph with every other private room deleted. Wiring `ensure_corridor` live (the actual go-live step for all of 4.1–4.4) surfaced this check finding real, previously-invisible placement defects — each traced to a specific root cause and fixed, not patched around:
  - A type-level MUST preference ("master_bedroom MUST bathroom") expands to an id-level edge against every matching instance, so an early version of the clustering pulled all 3 of 4bhk's bathrooms into one comb slot instead of just the ensuite — capped to one partner per type.
  - A more specific archetype can need more room along one axis than `zoned_bands`' general-purpose default for the *same* program (found symmetrically: `double_loaded_corridor` fit 4bhk where `zoned_bands` didn't; `zoned_bands` fit clinic where `double_loaded_corridor` didn't) — `generate_plan` now retries with `zoned_bands` once before refusing. Deliberately **not** extended to a "drop the corridor and accept the risk" fallback — that would let `generate_plan` return a plan with a real violation on a tight plot, silently breaking the guarantee this phase exists for; an honest `DoesNotFitError` is the correct outcome instead.
  - The corridor's own position is computed independently of the public band's internal subdivision, so their shared wall can end up a genuine sliver (0.738 m, below even the narrow-door floor) purely by coincidence of proportions — found via a 4-bedroom case where the corridor's only doors ended up being to the bedrooms it serves, an accidental dead end rather than a real bridge. New door-placement step guarantees a corridor always gets a door to *some* non-private neighbour, down to a physically-real 0.4 m floor, before the general spanning tree runs.
- [x] **One regression test's plot needed a bump, with reasoning shown, not silently patched:** `test_demo_three_bhk` (a real 30×40 ft / 9.144×12.192 m house) — this specific 4-genuinely-private-room program legitimately needs more depth than a bare 30×40 ft footprint to honour both the room program and the new guarantee at once. Verified the exact fitting boundary is 13.8 m before picking 14.0 m for a safety margin, rather than guessing.
- [x] **Verification:** all 5 real fixtures generate cleanly with zero violations; the Hypothesis property gate passes at its default 200 examples and was re-run manually at 2000 examples for extra confidence given the depth of placement logic touched; full backend suite **845 passed, 0 failed**, 3 expected live-LM skips; frontend unaffected (no schema/contract change) — tsc clean, 271 passed.
- [ ] **Not done (real, remaining scope):** a `spec.spaces`/non-residential corridor fixture end-to-end (blocked by nothing now, just not built); wiring `CIRCULATION_WIDTHS`'s commercial/accessible tiers to an actual context signal; door-clearance checks in front of doors (the roadmap's own stretch goal, never implemented in any phase).

### Phase 5.1 — Candidate search: generate-and-score ✅ (`phase3/layout-archetypes`)

> Continued automatically after Phase 4 completed, per explicit instruction. Scoped deliberately smaller than Phase 4 given how much that phase's "keep verifying before moving on" discipline ended up needing — one bounded, well-tested slice rather than the full doc vision.

- [x] **`plan_from_program` extracted from `generate_plan`** first (pure extraction, zero behavior change, verified against 108 engine/api/quality/archetype tests before building anything on top of it) — `generate_plan` is now just `plan_from_program(spec, _build_program(spec), ...)`. Lets search reuse the exact proven placement/door/room-assembly pipeline instead of reimplementing or risking drift from it.
- [x] **New `layout_engine/search.py`:** `generate_candidates(spec, n=64, seed=0)` produces N deterministic candidates via seeded room-order permutation of the same `EngineProgram`, ranked by one `energy()` function combining the existing `quality.scorer.score()` and `planning.graph_scoring.score_graph_satisfaction()` — both already built (Phase 6, Sprint 18) and reused directly via `layout_adapter.layout_plan_to_canvas` as the bridge, not reimplemented. Additive only — `generate_plan()` does not call any of this.
- [x] **Real, measured value:** best-of-64 improves 3bhk_adjacencies' quality score from 72 to 91 and 4bhk's from 92 to 100 — both regression-pinned as exact numbers, not just a `>=` bound. Determinism verified (same spec+seed → byte-identical ranked list). Runtime 0.03–0.25s per real fixture at n=64, well under the doc's 3s ceiling.
- [x] **Scoped to one variation axis** (seeded room-order permutation) **of the doc's four** — archetype parameter jitter (band share ±10%), a subdivision cut-axis bias, and splitting the candidate budget across the top-2 viable archetypes are named and deferred, not silently dropped. 5.2 (simulated-annealing refinement) is the doc's own "second PR, optional switch," not part of this slice.
- [x] **Tests:** 30 new (`test_mvp_search.py`, not `test_layout_benchmarks.py` — that file is the legacy `layout_service.py` engine's, this needed its own MVP-engine-side file, matching the established one-file-per-concern pattern). Every candidate validates clean, candidates rank in ascending energy, best-of-64 ≥ single-shot (the doc's stated acceptance bar) plus the two concrete score-improvement numbers above, determinism, runtime budget, and the `DoesNotFitError` boundary case.
- [x] **Verification:** full backend suite **875 passed** (845 baseline + 30 new), 0 failed, 3 expected live-LM skips; frontend unaffected (no schema change) — tsc clean.
...
- [x] **Phase 5.2 — simulated-annealing refinement, built and honestly benchmarked** (`phase5/fit-negotiation`): `_anneal()` in `search.py` runs a local search (single-swap neighbor moves on the room order, Metropolis acceptance, geometric cooling) starting from the best-of-n winner. Wired in as `anneal_iterations` on `generate_candidates`/`best_candidate`, default `0` (off) everywhere — nothing existing changes behavior. Same "can only take over when it demonstrably wins" pattern as everywhere else in this codebase: annealing tracks its best-seen plan separately from its (possibly-worse) current walk state, so it can never hand back anything worse than the best-of-n winner it started from.
- [x] **The doc's own prediction confirmed, not assumed:** annealed 200 extra iterations from the best-of-64 winner on all 5 fixtures — **zero improvement on every one** (`delta=+0.0`, identical plan). Confirmed this is real and not a bug in the annealing itself: annealing the same fixture from a deliberately worse starting point (the plain single-shot ordering, energy 1.10) converges to the exact same optimum best-of-64 already finds (energy 0.208) — the search works, there's just nowhere better left to go once best-of-64 has already found it. Left in the codebase as a tested, working, opt-in capability (not wired into the production router, since it costs ~5x more compute for zero measured gain today) rather than deleted, in case a future fixture with a rougher energy landscape shows a real need.
- [x] **Tests:** 4 new in `test_mvp_search.py` — never-worse-than-best-of-n (all 5 fixtures), determinism, off-by-default equivalence, and the from-a-worse-start convergence proof above.
- [x] **Verification:** `test_mvp_search.py` 39/39 passed (35 baseline + 4 new).
...
### Phase 9 — Engine hardening and golden benchmarks ✅ (`codex/phase9-engine-hardening`)

- [x] **Sanitary connectivity is structural, not a door-every-wall patch:** unattached bathrooms are distributed into corridor-served private comb clusters and each cluster is split along the corridor axis, so every member receives real corridor frontage. Wing balancing now uses required corridor frontage before area. Generated 3BHK/2-bath layouts pin a direct circulation door for both bathrooms; an explicitly requested ensuite MUST-edge remains a valid private-access exemption.
- [x] **The engine refuses invalid specialized-archetype output:** every completed single-floor plan is hard-validated. If `double_loaded_corridor`, `hub_and_spoke`, or `open_core` creates a privacy-chain or other hard violation, generation retries once through the proven `zoned_bands` safety comb; if that also cannot fit, it returns `DoesNotFitError` instead of emitting an invalid layout.
- [x] **Thirteen template-aligned golden fixtures:** studio apartment, apartment, family home, two-storey home, office, restaurant, retail, townhouse, villa, school, clinic, hotel, and warehouse fixtures exactly mirror the deterministic parser templates. Each stores a complete `LayoutPlan` snapshot plus a quality-score target with a ±2 gate, and every generated plan has zero hard violations.
- [x] **All-archetype property gate:** 80 deterministic randomized catalog-program/style cases cover all four archetypes and multiple candidate seeds. Every candidate is byte-deterministic, contained, pairwise non-overlapping, reachable, hard-valid, and respects the residential 1.0 m circulation-width floor.
- [x] **Golden bad-plan diagnostics:** a hand-built Entry → Bedroom → central Bathroom chain must report `through_room_access`, `generic.wet_room_exterior`, and `vastu.brahmasthan_occupied`. This pins the three workflow examples independently of generation and proves the explicit ensuite exemption separately.
- [x] **Editor and legacy integration:** all 13 fixtures, including one- through three-floor plans, survive derived wall/door removal and rebuild without losing room/floor identity or validity. Office, clinic, school, restaurant, and retail legacy layouts also pin that no room is left without an interior door.
- [x] **Benchmark output is part of CI:** `backend/scripts/benchmark_mvp_engine.py` prints a Markdown single-shot versus best-of-64 matrix for every fixture and fails if search regresses quality or exceeds 3 seconds. Best-of-64 is never worse than single-shot and the slowest measured fixture is about 0.2 seconds on the verification machine.
- [x] **Verification:** backend **1,011 passed, 3 expected live-model skips**; frontend **274 passed across 54 files**; `tsc --noEmit` and the 1,167-module production build passed. No dependency or migration was added.
- [ ] **Deliberate limits:** Phase 5.2 simulated annealing remains unbuilt because the committed benchmark does not show a measured need for another optimizer. School's current score of 68 is now a visible benchmark target rather than hidden drift. Multi-floor polygon boundaries remain explicitly unsupported pending their own geometry proof.

### Phase 10.1 — Hierarchical archetype composition ✅ (`codex/phase10-hierarchical-archetypes`)

- [x] **Per-band zone identity without legacy drift:** `BandEntry` carries `zone_id` while retaining the established two-value `(rect, rooms)` iteration contract. Every Phase 9 golden layout remains byte-identical; ordinary `PlanRoom.zone_id` and `LayoutPlan.archetype_reasons` values are omitted from serialization when absent.
- [x] **Conservative graph-driven activation:** a rectangular, single-floor program composes only when it has a real corridor spine and at least three repeated non-private, non-service units. MUST-attached service dependencies stay with their anchor; a dominant remaining room with a service dependency names the open-core region; remaining front/support spaces stay together. Repeated bedrooms and repeated bathrooms deliberately do not activate composition, so ordinary houses retain the Phase 9 path.
- [x] **One real shared spine:** a full-length corridor separates equal facing/back wings. Every macro-zone receives spans on both sides; repeat units alternate across the spine, while hub/open-core/public groups place their anchor and entry on the facing wing. Single-room comb slots along the corridor give every region literal circulation frontage instead of merely descriptive metadata.
- [x] **Door policy matches the geometry:** composed plans use required/MUST doors plus the circulation-rooted BFS tree, not the legacy convenience pass that doors every eligible shared wall. Existing required-door components are traversed before another corridor opening is added. The acceptance school uses 12 doors instead of the Phase 9 global plan's 23 while every room remains reachable; adjacent classroom partitions are pinned solid.
- [x] **Metadata survives the product boundary:** composed rooms persist `zone_id`; region reasons contain archetype keys, explanations, room IDs, and both zone spans. Backend and frontend canvas adapters preserve room `zoneId` and expose `metadata.archetypeReasons`, including canonical-to-editor-to-canonical round trips.
- [x] **Acceptance and benchmark:** the composed school fixture (four classrooms, hallway, gym, office/admin, dining/cafeteria, bathrooms, and storage) resolves to `double_loaded_corridor` plus `open_core` regions with zero hard violations from all four facings. Quality remains **100**, matching the explicit Phase 9 `zoned_bands` baseline, while doors fall **23 → 12** and best-of-64 takes about **0.19 s**. CI prints and enforces this Markdown benchmark.
- [x] **Fallbacks and verification:** a hierarchy that cannot fit silently returns to normal archetype selection; a composed result that fails a hard rule still takes the existing `zoned_bands` safety fallback. Full gates: backend **1,021 passed, 3 expected live-model skips**; frontend **275 passed across 54 files**; `tsc --noEmit` and the 1,167-module production build passed. No dependency or migration was added.
- [ ] **Deliberate limits:** this first accepted packet partitions repeat/service/core/support shapes, not arbitrary-depth connected subgraphs. Multi-floor layouts retain the aligned vertical-core path, polygon plots retain their separate polygon subdivider, explicit `layout_style` still requests one global archetype, and the editor stores region metadata without adding a new region-overlay UI.

### Integration — Phase 5.2/5.3-10.1 merged into `main` (`integration/phase5-10.1`, 2026-08-09)

> Phases 5.3 through 10.1 above were built by Codex on stacked branches off `origin/main`'s tip and sat unmerged, undiscovered by this file's own history, for over a week. Found during a full branch/status audit, reviewed, reconciled with an independently-developed set of bug fixes touching the same files, and merged — closing the gap between what the roadmap had actually delivered and what `main` reflected.

- [x] **Deep review before merge, not a rubber stamp:** every phase unique to the codex chain got the same from-scratch adversarial pass as `main`'s own code gets. Found and fixed 4 more real bugs: Phase 7 multi-floor could misalign reused stair/lift cores across floors and silently return an entire unreachable storey (`_generate_plan_multifloor` never called `validate()` at all, despite its own docstring claiming otherwise); Phase 8's constraint endpoints being catalog-key-canonicalized while a `spec.rooms`-sourced plan still emits raw `RoomType` values meant a genuinely satisfied MUST adjacency scored as unmet (39-point false penalty, and it silently corrupted `search.energy()`'s ranking too); Phase 8's `ensure_registered` had no bound on total requested rooms before graph construction, letting one request burn 36s CPU / 237MB and permanently pollute the shared process-global catalog; the Phase 9/10.1 leaf min-size gate sat outside the retry envelope it was supposed to be inside, refusing 14 of 600 sampled programs that `zoned_bands` demonstrably fit on the same plot.
- [x] **Phase 5.2 — simulated annealing, built and honestly benchmarked:** the one item Phase 5.1 left deliberately deferred. Local search (single-swap neighbor moves, Metropolis acceptance, geometric cooling) starting from the best-of-64 winner. Opt-in (`anneal_iterations`, default 0), can never score worse than the winner it started from. Benchmarked, not assumed: zero improvement over best-of-64 on all 5 fixtures — confirmed this is real (nowhere better to go from an already-good start), not a broken search, by annealing the same fixture from a deliberately worse starting point and watching it converge to the identical optimum best-of-64 already finds.
- [x] **5 real merge conflicts, resolved by understanding both sides, not by picking one blindly:** `schemas/layout_plan.py` (Phase 10.1's `archetype_reasons` metadata + the security fix's `max_length` payload caps — both needed); `layout_engine/engine.py` (an older privacy-chain retry loop built on pre-Phase-9 `main`, superseded by Phase 9/10.1's more general `validate()`-based retry — kept the current one, deleted the superseded one as dead code rather than leaving two competing implementations); `layout_engine/search.py` (two independent real fixes — DoS-guard ordering and polygon-boundary handling — combined so the size guard runs before graph construction on *both* branches, since a polygon spec can carry just as large a `spec.spaces` list as a rectangular one); `mvpLayoutAdapter.ts` (the codex chain had already fixed the same closed-room-type-whitelist bug found independently, plus additionally covered stair objects for multi-floor — kept the more complete version).
- [x] **Post-merge fixture staleness caught, not silently ignored:** 4 golden-snapshot tests (`two_storey_home`, `townhouse`, `villa`, `hotel` — all multi-floor) failed on exact door-offset equality after the merge. Verified before touching anything: all 4 regenerate with zero hard violations and identical quality scores to the old snapshot — the stair/lift canonical-sizing fix from the review legitimately shifts multi-floor door placement; the old snapshots simply predated it. Regenerated those 4 entries.
- [x] **Verification:** backend **1,051 passed, 3 expected live-model skips, 0 failed**; frontend **281 passed across 54 files**; `tsc --noEmit` clean; production build clean (1,167 modules).
- [x] **Phase 10.2 — non-rectangular plot/room support in the frontend** (`phase10/nonrect-frontend`): closes the gap flagged above. `contracts.ts` gained `Vertex`/`PlanRoom.vertices`/`PlotSpec.boundary`, mirroring the backend schema field-for-field. `canvasStore.ts`'s `Room` gained `polygonVertices` (world x/z, absolute — a polygon room's rotation is always 0 backend-side, so unlike a rectangular room's size, there's no local/world transform to undo). `mvpLayoutAdapter.ts` carries vertices both directions: `layoutPlanToCanvas` shifts them by the same delta `boundedCenter` applies so the outline stays consistent with its own clamped bbox; `canvasObjectsToLayoutPlan` derives a polygon room's `x/y/w/h` from the vertices themselves (the actual source of truth the backend validates against), not from `position`/`size`. `Plan2DObject.tsx` renders an SVG `<polygon>` in place of `<rect>` for a room with vertices, with per-vertex drag handles (footprint-clamped, grid-snapped, same transient-drag-then-single-history-entry pattern as every other move/resize) replacing the standard 8-point handles, plus double-click-an-edge to insert a vertex and double-click-a-vertex to remove one (refused below 3). Cherry-picked one prerequisite backend fix from the abandoned `codex/phase10-nonrect-frontend` branch first: `rebuild_derived_geometry` was ignoring `vertices`/`boundary` and rebuilding walls as if every room were still rectangular, silently corrupting an edited polygon layout on save.
- [x] **Scoped deliberately to 2D Plan only:** matches the backend's own precedent (2D SVG export is the proven polygon path); the 3D view's resize grips are built entirely on axis-aligned box assumptions and would need their own slice. Editing `PlotSpec.boundary` itself (vs. individual room outlines) is left for later — it's normally set at generation time, a rarer interactive-editing need than room shape.
- [x] **Verification:** 15 new `plan2dGeometry` tests + 1 adapter round-trip test. Frontend **288 passed / 54 files**, `tsc --noEmit` clean, production build clean. Backend **1054 passed, 3 expected live-model skips, 0 failed** (frontend-only change; confirms no incidental regression). Local merge only — not pushed to `origin`, pending owner review.
