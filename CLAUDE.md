...
- *Slice 4 — daylight / external-wall awareness:* `score_graph_satisfaction` now also measures, per generated layout, how many spaces that need an exterior wall (`Node.requires_external_wall`, already set for bedrooms/living/office/etc.) actually reach the building perimeter — surfaced in `metadata.graphSatisfaction` (`daylightTotal`/`daylightSatisfied`/`daylightMissing`). Pure measurement + explainable warning, zero placement risk. It immediately earned its keep: it caught that the **BSP back-partition landlocks rooms** (a 3-BR house scored daylight 5/8 with Meeting Room/Office/Bedroom fully interior), while office/school layouts scored full marks.

No existing candidate is generated differently and none removed. Full backend suite 512 passed, zero regressions.

Deferred (Phase 4 remainder): daylight-aware candidate *selection* (penalise landlocked-room layouts — the natural fix for the BSP finding above, deferred as a selection change with its own regression surface), `preferred_relative_position` honouring (needs the parser to capture direction, which it currently drops), and true 2D BSP footprint slicing by adjacency cluster. The scoring + selection + row-clustering engine + daylight measurement land first; the full spatial solver is the larger follow-up.
