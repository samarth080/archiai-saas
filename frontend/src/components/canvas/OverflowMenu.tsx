...
export function OverflowMenu({
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-10 z-40 w-56 rounded-2xl border border-ink/10 bg-graphite-800 p-1.5 shadow-2xl">
          ...
          {exportError && <p className="px-2.5 py-1 text-xs text-danger">{exportError}</p>}
          <div className="my-1 h-px bg-ink/10" />
          <button type="button" disabled={duplicating} onClick={run(onDuplicate)} className={rowClass}>
            {duplicating ? 'Duplicating…' : 'Duplicate'}
          </button>
          {duplicateError && <p className="px-2.5 py-1 text-xs text-danger">{duplicateError}</p>}
          ...
          <button
            ...
            className="flex w-full items-center justify-between gap-2 rounded-lg px-2.5 py-2 text-left text-sm font-medium text-danger hover:bg-danger/10 disabled:cursor-not-allowed disabled:opacity-40"
          >
            {deleting ? 'Deleting…' : 'Delete'}
          </button>
          {deleteError && <p className="px-2.5 py-1 text-xs text-danger">{deleteError}</p>}
        </div>
      )}
    </div>
  )
