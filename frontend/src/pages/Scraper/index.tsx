  ...
  return (
    <div className="flex h-screen bg-surface">
      ...
      <main className="flex-1 overflow-y-auto p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-ink">Internal Data Pipeline</h1>
        ...
        <section className="mb-6 rounded-lg border border-ink/10 bg-graphite-800 p-4">
          <h2 className="mb-4 text-base font-semibold text-ink">Add Public Text Source</h2>
        ...
        {loading ? (
          <p className="py-12 text-center text-muted-light">Loading pipeline...</p>
        ) : (
          <>
            <section className="mb-6 rounded-lg border border-ink/10 bg-graphite-800">
              <div className="flex items-center justify-between border-b border-ink/10 px-4 py-3">
                <h2 className="text-base font-semibold text-ink">Sources</h2>
                <span className="text-xs text-muted-light">{sources.length} registered</span>
              </div>
              {sources.length === 0 ? (
                <p className="px-4 py-8 text-center text-sm text-muted-light">No public text sources registered.</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-ink/10 text-sm">
                    <thead className="bg-surface text-left text-xs uppercase text-muted-light">
                      <tr>
                      ...
                      {sources.map((source) => (
                        <tr key={source.id}>
                          <td className="px-4 py-3">
                            ...
                            <p className="max-w-md truncate text-xs text-muted-light">{source.base_url}</p>
                          </td>
                          <td className="px-4 py-3 text-muted">{source.source_category}</td>
                          <td className="px-4 py-3 text-muted">{formatDate(source.last_checked)}</td>
                          <td className="px-4 py-3">
            ...
            <div className="mb-6 grid gap-6 xl:grid-cols-2">
              <section className="rounded-lg border border-ink/10 bg-graphite-800">
                <div className="border-b border-ink/10 px-4 py-3">
                  <h2 className="text-base font-semibold text-ink">Latest Status</h2>
                </div>
                {!latestRun ? (
                  <p className="px-4 py-8 text-sm text-muted-light">No scraper runs yet.</p>
                ) : (
                  <div className="space-y-2 px-4 py-4 text-sm text-muted">
                    ...
                    {latestRun.error_message && <p className="text-danger">{latestRun.error_message}</p>}
                  </div>
                )}
              </section>
                  ...
                  <ul className="divide-y divide-ink/10">
                    {runs.slice(0, 8).map((run) => (
                      <li key={run.id} className="flex items-center justify-between gap-3 px-4 py-3 text-sm">
                        <div>
                          <p className="font-medium text-muted">{formatDate(run.started_at)}</p>
                          <p className="text-xs text-muted-light">{run.records_collected} records</p>
                        </div>
            ...
            <section className="rounded-lg border border-ink/10 bg-graphite-800">
              <div className="flex items-center justify-between border-b border-ink/10 px-4 py-3">
                <h2 className="text-base font-semibold text-ink">Extracted Layout Patterns</h2>
                <span className="text-xs text-muted-light">{patterns.length} references</span>
              </div>
              {patterns.length === 0 ? (
                <p className="px-4 py-8 text-center text-sm text-muted-light">No structured patterns extracted yet.</p>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-ink/10 text-sm">
                    <thead className="bg-surface text-left text-xs uppercase text-muted-light">
                    ...
                    <tbody className="divide-y divide-ink/10">
                      {patterns.slice(0, 20).map((pattern) => (
                        <tr key={pattern.id}>
                          <td className="px-4 py-3 font-medium text-ink">{pattern.room_type ?? 'General'}</td>
                          <td className="px-4 py-3 text-muted">{pattern.building_type ?? '-'}</td>
                          <td className="px-4 py-3 text-muted">
                            {pattern.typical_area_sqm_min === null
                              ? '-'
                              : `${pattern.typical_area_sqm_min}-${pattern.typical_area_sqm_max}`}
                          </td>
                          <td className="px-4 py-3 text-muted">{pattern.zone ?? '-'}</td>
                          <td className="px-4 py-3 text-muted">{pattern.confidence}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
